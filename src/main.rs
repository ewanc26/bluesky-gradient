//! Generate gradient images for Bluesky profiles and banners.
//!
//! For each hour of the day this produces a PNG whose sky colour interpolates
//! through a configurable palette, then fades to a muted monochrome at the
//! bottom edge. A noise pass gives the flat fills a slight film-grain texture.
...
fn main() -> Result<()> {
    ...
    // Auto-increment the folder name so multiple runs don't collide.
    let output_folder = config::get_available_folder(&params.output_folder_base);
    fs::create_dir_all(&output_folder)?;

    // ── Generate one image per hour ───────────────────────────────────
    // Already-existing files are skipped, making re-runs idempotent.
    for hour in 0..24 {
